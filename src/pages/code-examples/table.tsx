...
function TableActions({ post }: { post: Post }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <InlineIconButton label="Delete" icon={CloseIcon} color="error" onPress={handleDelete} />
      <InlineIconButton label="View" icon={ViewIcon} color="primary" onPress={handleView} />
    </div>
  )
}
...
function TableExamplesPage() {
  return (
    <div className="p-20 flex flex-col gap-4">
      <Table items={posts} columns={columnDefinitions} />
    </div>
  )
}
